...
База данных тоже будет создана в docker под названием `skilla` резервирует порт 3333, подключиться к БД можно через
любой клиент по порту `3333` `-u root -p root`
...
Перед началом работы с API необходимо создать RSA ключи и клиента Laravel Passport

```bash
docker exec -i php8.3_skilla php artisan passport:keys
```
Клиентов API можно создать для двух моделей `Users` и `Worker`, вторые для выпуска токенов и авторизации в сокетах
